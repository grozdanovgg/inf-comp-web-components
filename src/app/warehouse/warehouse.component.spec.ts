...
describe('WarehouseComponent', () => {
  ...
  beforeEach(() => {
    ...
    component.ships = Array.from(Array(10).keys())
      .map((value, index) => {
        return new Ship({
  ...
  it('should initialize some default properties', () => {
  ...
  describe('When focus changes in the slider', () => {
    it('set the currentShip', fakeAsync(() => {
      const ship = new Ship({ ShipID: 'fooId' } as Ship);
      component.onShipChangeInSlider(ship);
      tick(1);

      expect(component.focusedShip).toEqual(ship);
    }));
  });

  describe('transferShipToGame()', () => {
    it('', () => {
